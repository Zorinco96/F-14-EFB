...
def test_default_interface_uses_neutral_status_and_coarse_planning_outputs():
    ...
    assert _element(app.metric, "Distance allowance").value.endswith("NM")
...
def test_henderson_observation_is_shown_without_a_go_call():
    ...
    app.run()
    _element(app.selectbox, "Takeoff thrust rating").set_value("DERATE 1")
    app.run()

    assert not app.exception
    assert _element(app.metric, "Departure").value == "PLANNING HOLD"
    assert _element(app.metric, "SET FF / engine").value == "5,250 PPH"
